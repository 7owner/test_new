...
SELECT 'Capteur de temp̩rature', 'TEMP001', 'Capteur de temp̩rature ambiante', 'GTB', 'Siemens', 'Fournisseur A', 0, 'Classe A', 50.00, 'Capteur haute pr̩cision', 'GTB', TRUE
WHERE NOT EXISTS (SELECT 1 FROM materiel_catalogue WHERE reference = 'TEMP001');

INSERT INTO materiel_catalogue (titre, reference, designation, categorie, fabricant, fournisseur, remise_fournisseur, classe_materiel, prix_achat, commentaire, metier, actif)
SELECT 'Cam̩ra d̫me IP', 'CAMIP001', 'Cam̩ra de surveillance IP d̫me 5MP', 'Video', 'Hikvision', 'Fournisseur B', 10, 'Classe B', 250.00, 'Vision nocturne IR 30m', 'Video', TRUE
WHERE NOT EXISTS (SELECT 1 FROM materiel_catalogue WHERE reference = 'CAMIP001');

INSERT INTO materiel_catalogue (titre, reference, designation, categorie, fabricant, fournisseur, remise_fournisseur, classe_materiel, prix_achat, commentaire, metier, actif)
SELECT 'Lecteur de badges', 'RFID001', 'Lecteur de badges RFID multi-fr̩quences', 'Control_Acces', 'Vanderbilt', 'Fournisseur C', 5, 'Classe A', 150.00, 'Compatible MIFARE/DESFire', 'Control_Acces', TRUE
WHERE NOT EXISTS (SELECT 1 FROM materiel_catalogue WHERE reference = 'RFID001');

-- Seed mat̩riels (commandes)
...
SELECT 'Capteur de temp̩rature', 'TEMP001', 'Capteur de temp̩rature ambiante', 'GTB', 'Siemens', 'Fournisseur A', 0, 'Classe A', 50.00, 'Commande pour site Paris 1', 'A commander', 'GTB', (SELECT id FROM agence WHERE titre = 'Agence Paris' LIMIT 1)
WHERE NOT EXISTS (SELECT 1 FROM materiel WHERE reference = 'TEMP001' AND commentaire = 'Commande pour site Paris 1');

INSERT INTO materiel (titre, reference, designation, categorie, fabricant, fournisseur, remise_fournisseur, classe_materiel, prix_achat, commentaire, commande_status, metier, agence_id)
SELECT 'Cam̩ra d̫me IP', 'CAMIP001', 'Cam̩ra de surveillance IP d̫me 5MP', 'Video', 'Hikvision', 'Fournisseur B', 10, 'Classe B', 250.00, 'Commande pour site Lyon 1', 'En livraison', 'Video', (SELECT id FROM agence WHERE titre = 'Agence Lyon' LIMIT 1)
...
SELECT c.id, s.id, 'Demande de capteurs', 'Besoin de 5 capteurs de temp̩rature suppl̩mentaires', 'En cours de traitement'
...
INSERT INTO demande_client (client_id, site_id, titre, description, status)
SELECT c.id, s.id, 'Probl̬me cam̩ra', 'Une cam̩ra HS sur Site Lyon 1', 'En cours de traitement'
FROM (SELECT id FROM client WHERE nom_client = 'Client ACME' LIMIT 1) c,
     (SELECT id FROM site WHERE nom_site = 'Site Lyon 1' LIMIT 1) s
WHERE NOT EXISTS (SELECT 1 FROM demande_client WHERE titre = 'Probl̬me cam̩ra');

-- Seed interventions (associ̩es aux demandes client)
...
INSERT INTO intervention (ticket_id, demande_id, description, date_debut, status, metier)
SELECT t.id, dc.id, 'Intervention remplacement cam̩ra', CURRENT_DATE, 'En_cours', 'Video'
FROM (SELECT id FROM ticket WHERE titre = 'Ticket Urgente' LIMIT 1) t,
     (SELECT id FROM demande_client WHERE titre = 'Probl̬me cam̩ra' LIMIT 1) dc
WHERE NOT EXISTS (SELECT 1 FROM intervention WHERE description = 'Intervention remplacement cam̩ra');

-- Seed intervention_materiel (utilisant les materiels command̩s)
...
INSERT INTO intervention_materiel (intervention_id, materiel_id, quantite, commentaire)
SELECT i.id, m.id, 1, 'Cam̩ra pour remplacement'
FROM (SELECT id FROM intervention WHERE description = 'Intervention remplacement cam̩ra' LIMIT 1) i,
     (SELECT id FROM materiel WHERE reference = 'CAMIP001' LIMIT 1) m
WHERE NOT EXISTS (SELECT 1 FROM intervention_materiel WHERE commentaire = 'Cam̩ra pour remplacement');

-- Seed du table rapport_ticket
INSERT INTO rapport_ticket (ticket_id, matricule, commentaire_interne, etat)
SELECT t.id, 'AGT001', 'Rapport initial apr̬s diagnostic.', 'En_cours'
FROM (SELECT id FROM ticket WHERE titre = 'Ticket Semaine 42' LIMIT 1) t
WHERE NOT EXISTS (SELECT 1 FROM rapport_ticket WHERE ticket_id = t.id);

INSERT INTO rapport_ticket (ticket_id, matricule, commentaire_interne, etat)
SELECT t.id, 'AGT002', 'Probl̬me r̩solu. Fermeture du ticket.', 'Termine'
FROM (SELECT id FROM ticket WHERE titre = 'Ticket Urgente' LIMIT 1) t
...
((SELECT id FROM doe WHERE titre='DOE Paris 2025' LIMIT 1), (SELECT id FROM affaire WHERE nom_affaire='Contrat Ticket ACME' LIMIT 1), (SELECT id FROM site WHERE nom_site='Site Paris 1' LIMIT 1), (SELECT id FROM demande_client WHERE titre='Demande de capteurs' LIMIT 1), 'Installation capteurs Hall A', 'Installation de 5 capteurs de temp̩rature dans le Hall A.', 'A_faire', 'Haute', '2025-04-01 09:00', NULL, '2025-04-05 17:00'),
((SELECT id FROM doe WHERE titre='DOE Paris 2025' LIMIT 1), (SELECT id FROM affaire WHERE nom_affaire='Contrat Ticket ACME' LIMIT 1), (SELECT id FROM site WHERE nom_site='Site Lyon 1' LIMIT 1), (SELECT id FROM demande_client WHERE titre='Probl̬me cam̩ra' LIMIT 1), 'Remplacement cam̩ra site Lyon 1', 'Remplacement de la cam̩ra d̩fectueuse.', 'En_cours', 'Moyenne', '2025-04-10 10:00', NULL, '2025-04-12 17:00'),
(NULL, NULL, (SELECT id FROM site WHERE nom_site='Site Paris 1' LIMIT 1), NULL, 'V̩rification g̩n̩rale site Paris', 'V̩rification annuelle des installations.', 'Termine', 'Basse', '2025-03-20 08:00', '2025-03-20 16:00', NULL);
...
((SELECT id FROM demande_client WHERE titre='Probl̬me cam̩ra' LIMIT 1), (SELECT id FROM travaux WHERE titre='Remplacement cam̩ra site Lyon 1' LIMIT 1));
...
((SELECT id FROM travaux WHERE titre='Remplacement cam̩ra site Lyon 1' LIMIT 1), 'AGT002', '2025-04-10 10:00'),
((SELECT id FROM travaux WHERE titre='V̩rification g̩n̩rale site Paris' LIMIT 1), 'AGT003', '2025-03-20 08:00');
...
((SELECT id FROM travaux WHERE titre='Remplacement cam̩ra site Lyon 1' LIMIT 1), 'AGT001', 'Secondaire');

-- Travaux Tache
INSERT INTO travaux_tache (travaux_id, titre, description, etat, priorite, date_echeance) VALUES
((SELECT id FROM travaux WHERE titre='Installation capteurs Hall A' LIMIT 1), 'Pr̩paration du mat̩riel', 'Rassembler les capteurs et outils n̩cessaires.', 'En_cours', 'Haute', '2025-04-01 12:00'),
((SELECT id FROM travaux WHERE titre='Installation capteurs Hall A' LIMIT 1), 'C̢blage des capteurs', 'Proc̩der au c̢blage des nouveaux capteurs.', 'A_faire', 'Haute', '2025-04-03 17:00'),
((SELECT id FROM travaux WHERE titre='Remplacement cam̩ra site Lyon 1' LIMIT 1), 'D̩pose ancienne cam̩ra', 'Retirer la cam̩ra d̩fectueuse du mur.', 'En_cours', 'Moyenne', '2025-04-10 12:00');

-- Travaux Materiel
INSERT INTO travaux_materiel (travaux_id, materiel_id, quantite, commentaire) VALUES
((SELECT id FROM travaux WHERE titre='Installation capteurs Hall A' LIMIT 1), (SELECT id FROM materiel WHERE reference='TEMP001' LIMIT 1), 5, 'Capteurs command̩s pour l''installation'),
((SELECT id FROM travaux WHERE titre='Remplacement cam̩ra site Lyon 1' LIMIT 1), (SELECT id FROM materiel WHERE reference='CAMIP001' LIMIT 1), 1, 'Cam̩ra de remplacement');

-- Rendu Travaux
INSERT INTO rendu_travaux (travaux_id, resume, valeur) VALUES
((SELECT id FROM travaux WHERE titre='V̩rification g̩n̩rale site Paris' LIMIT 1), 'V̩rification annuelle termin̩e avec succ̬s.', 'Tous les syst̬mes ont ̩t̩ v̩rifi̩s et sont conformes et op̩rationnels.');

-- Rendu Travaux Image (references the dummy image with id=1)
INSERT INTO rendu_travaux_image (rendu_travaux_id, image_id) VALUES
((SELECT id FROM rendu_travaux WHERE travaux_id=(SELECT id FROM travaux WHERE titre='V̩rification g̩n̩rale site Paris' LIMIT 1) LIMIT 1), 1);
...
UPDATE demande_materiel SET travaux_id = (SELECT id FROM travaux WHERE titre='Remplacement cam̩ra site Lyon 1' LIMIT 1) WHERE titre='Probl̬me cam̩ra';
