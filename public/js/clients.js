document.addEventListener('DOMContentLoaded', () => {
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      clients.forEach(client => {
        ...
        clientEl.innerHTML = `
          <div>
            <p class="font-semibold"><a href="/client-view.html?id=${client.id}">${client.nom_client}</a></p>
  ...
  const loadAdressesIntoSelectForClient = async () => {
    const token = localStorage.getItem('token');
    try {
      const response = await fetch('/api/adresses', { headers: { 'Authorization': `Bearer ${token}` } });
      if (!response.ok) return;
    ...
    } catch (error) { console.error('Error fetching adresses for select:', error); }
  };
