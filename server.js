...
app.get('/api/sites/:id/relations', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const maintenances = (await pool.query('SELECT m.* FROM maintenance m JOIN doe d ON m.doe_id=d.id WHERE d.site_id=$1 ORDER BY m.id DESC', [id])).rows;
    ...
    res.json({ site, adresse, affaires, does, maintenances, rendezvous, documents, images });
  } catch (err) {
...
app.get('/api/does/:id/relations', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const maintenances = (await pool.query('SELECT * FROM maintenance WHERE doe_id=$1 ORDER BY id DESC', [id])).rows;
    ...
    res.json({ doe, maintenances, documents, images });
  } catch (err) {
...
app.get('/api/maintenances/:id/relations', authenticateToken, async (req, res) => {
  const { id } = req.params;
  try {
    const m = (await pool.query('SELECT * FROM maintenance WHERE id=$1', [id])).rows[0];
    if (!m) return res.status(404).json({ error: 'Maintenance not found' });
    ...
    const interventions = (await pool.query('SELECT * FROM intervention WHERE maintenance_id=$1 ORDER BY id DESC', [id])).rows;
    const documents = (await pool.query("SELECT * FROM documents_repertoire WHERE cible_type='Maintenance' AND cible_id=$1 ORDER BY id DESC", [id])).rows;
    const images = (await pool.query("SELECT id, nom_fichier, type_mime FROM images WHERE cible_type='Maintenance' AND cible_id=$1 ORDER BY id DESC", [id])).rows;
    res.json({ maintenance: m, doe, affaire, site, interventions, documents, images });
  } catch (err) {
    console.error('Error fetching maintenance relations:', err);
    res.status(500).json({ error: 'Internal Server Error' });
  }
});
...
app.get('/api/interventions/:id/relations', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const maintenance = intervention.maintenance_id
      ? (await pool.query('SELECT * FROM maintenance WHERE id=$1', [intervention.maintenance_id])).rows[0]
      : null;
    let doe = null; let site = null; let affaire = null;
    if (maintenance && maintenance.doe_id) {
      doe = (await pool.query('SELECT * FROM doe WHERE id=$1', [maintenance.doe_id])).rows[0] || null;
      if (doe && doe.site_id) {
        site = (await pool.query('SELECT * FROM site WHERE id=$1', [doe.site_id])).rows[0] || null;
      }
    }
    if (maintenance && maintenance.affaire_id) {
      affaire = (await pool.query('SELECT * FROM affaire WHERE id=$1', [maintenance.affaire_id])).rows[0] || null;
    }
    ...
    res.json({ intervention, maintenance, doe, site, affaire, rendezvous, documents, images });
  } catch (err) {
    ...
    if (overview) {
      const result = await pool.query(`
        SELECT 
          ...
          COALESCE((SELECT COUNT(*) FROM maintenance m JOIN doe d2 ON m.doe_id = d2.id WHERE d2.site_id = s.id), 0) AS maintenances_count,
          (SELECT MAX(m.date_debut) FROM maintenance m JOIN doe d3 ON m.doe_id = d3.id WHERE d3.site_id = s.id) AS last_maintenance_date
        FROM site s
...
app.put('/api/sites/:id', authenticateToken, async (req, res) => {
  const { id } = req.params; const { nom_site, adresse_id, commentaire } = req.body;
  try {
    const result = await pool.query('UPDATE site SET nom_site=$1, adresse_id=$2, commentaire=$3 WHERE id=$4 RETURNING *', [nom_site, adresse_id || null, commentaire || null, id]);
    res.json(result.rows[0] || null);
  } catch (err) {
...
// Link Site <-> Affaire
app.post('/api/site_affaire', authenticateToken, async (req, res) => {
...
app.delete('/api/site_affaire', authenticateToken, async (req, res) => {
  const { site_id, affaire_id } = req.body;
  try {
...
app.post('/api/equipes', authenticateToken, async (req, res) => {
...
app.put('/api/equipes/:id', authenticateToken, async (req, res) => {
...
app.delete('/api/equipes/:id', authenticateToken, async (req, res) => {
  const { id } = req.params;
...
app.post('/api/agence_membres', authenticateToken, async (req, res) => {
...
app.put('/api/agence_membres/:id', authenticateToken, async (req, res) => {
...
app.delete('/api/agence_membres/:id', authenticateToken, async (req, res) => {
...
app.post('/api/agent_equipes', authenticateToken, async (req, res) => {
...
app.delete('/api/agent_equipes/:id', authenticateToken, async (req, res) => {
...
app.post('/api/fonctions', authenticateToken, async (req, res) => {
...
app.put('/api/fonctions/:id', authenticateToken, async (req, res) => {
...
app.delete('/api/fonctions/:id', authenticateToken, async (req, res) => {
...
app.post('/api/agent_fonctions', authenticateToken, async (req, res) => {
...
app.put('/api/agent_fonctions/:id', authenticateToken, async (req, res) => {
...
app.delete('/api/agent_fonctions/:id', authenticateToken, async (req, res) => {
...
app.put('/api/achats/:id', authenticateToken, async (req, res) => {
...
app.delete('/api/achats/:id', authenticateToken, async (req, res) => {
...
app.post('/api/reglements', authenticateToken, async (req, res) => {
  try {
...
app.delete('/api/reglements/:id', authenticateToken, async (req, res) => {
